...
namespace VoxelLandscapeEditor
{
    public class World
    {
        public const int X_CHUNKS = 25, Y_CHUNKS = 25, Z_CHUNKS = 1;
        public const int X_SIZE = X_CHUNKS * Chunk.X_SIZE;
        public const int Y_SIZE = Y_CHUNKS * Chunk.Y_SIZE;
        public const int Z_SIZE = Z_CHUNKS * Chunk.Z_SIZE;

        const double REDRAW_INTERVAL = 60;

        public Chunk[, ,] Chunks = new Chunk[X_CHUNKS, Y_CHUNKS, Z_CHUNKS];
        ...
        public void SetVoxel(int x, int y, int z, bool active, Color top, Color side)
        {
            if (x < 0 || y < 0 || z < 0 || x >= X_SIZE || y >= Y_SIZE || z >= Z_SIZE) return;

            Chunk c = GetChunkAtWorldPosition(x, y, z);

            c.Voxels[x - ((x / Chunk.X_SIZE) * Chunk.X_SIZE), y - ((y / Chunk.Y_SIZE) * Chunk.Y_SIZE), z - ((z / Chunk.Z_SIZE) * Chunk.Z_SIZE)] = new Voxel(active, top, side);
        ...
        {
            int height = 3 + Helper.Random.Next(5);
            for (int h = z; h > z - height; h--)
            {
                SetVoxel(x,y,h,true, Color.SaddleBrown, Color.SaddleBrown);
                ...
                if (h < z - 3)
                {
                    for (int xx = x - 1; xx <= x + 1; xx++)
                    {
                        for (int yy = y - 1; yy <= y + 1; yy++)
                        {
                            ...
                            {
                                SetVoxel(xx, yy, h, true, new Color(0.2f, 0.8f + ((float)Helper.Random.NextDouble() * 0.1f), 0.2f), new Color(0.2f, 0.5f + ((float)Helper.Random.NextDouble() * 0.1f), 0.2f));
                            }
                        }
                    }
                }
            ...
            SetVoxel(x, y, z - (height), true, new Color(0.2f, 0.8f + ((float)Helper.Random.NextDouble() * 0.1f), 0.2f), new Color(0.2f, 0.5f + ((float)Helper.Random.NextDouble() * 0.1f), 0.2f));
